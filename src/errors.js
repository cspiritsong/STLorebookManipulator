...
const RULES = [
    {
        match: ['could not parse', 'invalid json', 'not valid json', 'unexpected token', 'invalid xml', 'no results from ai', 'json', 'xml'],
    ...
    {
        match: ['no connection', 'no api', 'api is not connected', 'select a connection profile that has an api', 'onlinestatus', 'no_connection'],
    ...
    {
        match: ['401', '403', 'unauthorized', 'forbidden', 'api key', 'invalid key', 'authentication'],
    ...
    {
        match: ['context length', 'context window', 'too long', 'maximum context', 'token limit exceeded'],
    ...
    {
        match: ['timeout', 'timed out', 'etimedout', 'network', 'fetch failed', 'failed to fetch', 'econnrefused'],
