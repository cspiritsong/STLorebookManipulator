...
assert(typeof escapeHtml === 'function', 'escapeHtml is exported and importable');
assert(typeof escapeAttr === 'function', 'escapeAttr is exported and importable');
...
assert(escapeHtml('"quoted"') === '&quot;quoted&quot;', 'escapeHtml escapes double quotes');
...
assert(escapeHtml(undefined) === '', 'escapeHtml returns empty string for undefined');

// escapeHtml escapes ampersand before other entities (no double-escaping)
assert(escapeHtml('<') === '&lt;', 'escapeHtml does not double-escape angle bracket');

// escapeAttr escapes quotes for attribute safety
assert(escapeAttr('say "hi"') === 'say &quot;hi&quot;', 'escapeAttr escapes double quotes');
assert(escapeAttr("it's mine") === 'it&#39;s mine', 'escapeAttr escapes single quotes');

// escapeAttr handles null/undefined gracefully
assert(escapeAttr(null) === '', 'escapeAttr returns empty string for null');
assert(escapeAttr(undefined) === '', 'escapeAttr returns empty string for undefined');
...
assert(!escaped.includes('"'), 'escapeAttr removes raw double quotes that would break attributes');
