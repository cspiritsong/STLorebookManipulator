...
function hasShape(info) {
    return typeof info.title === 'string' && info.title.length > 0
        && typeof info.what === 'string' && info.what.length > 0
        && typeof info.fix === 'string' && info.fix.length > 0;
}

// JSON / parsing failures
const jsonErr = explainError(new Error('Could not parse LLM response as JSON.'));
...
assert(/format/i.test(xmlErr.title), 'Invalid XML phrasing maps to format guidance');

// Connection Manager missing
const cmErr = explainError(new Error('Connection Manager is not available'));
assert(/connection manager/i.test(cmErr.title), 'Connection Manager error recognized');
...
assert(/no ai connection/i.test(noConnErr.title), 'No-connection error recognized');

// Quota / rate limit
const quotaErr = explainError(new Error('Request failed with status 429: quota exceeded'));
assert(/rate-limit|quota/i.test(quotaErr.title), 'Quota/rate-limit error recognized');
...
// Context length
const ctxErr = explainError(new Error('This model maximum context length exceeded'));
assert(/too big|context/i.test(ctxErr.title), 'Context-length error recognized');

// Network/timeout
const netErr = explainError(new Error('fetch failed: ETIMEDOUT'));
assert(/reach the ai|network/i.test(netErr.title), 'Network/timeout error recognized');
...
// Storage
const storageErr = explainError(new Error('Backup failed: storage may be full'));
...
assert(unknownErr.raw === 'Kaboom 12345 weird thing', 'Unknown error preserves raw detail');
...
assert(html.includes('lm-friendly-error'), 'Rendered HTML has the friendly-error container');
...
const xssHtml = renderFriendlyError(new Error('<script>alert(1)</script>'), escapeHtml);
assert(!xssHtml.includes('<script>'), 'Raw error text is escaped (no live script tag)');
