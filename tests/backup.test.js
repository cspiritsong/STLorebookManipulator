...
assert(
  JSON.stringify(backup1.data) === JSON.stringify(SAMPLE_DATA),
...
assert(
  history1[0].timestamp === backup1.timestamp,
  "History returns correct backup",
);

// Create second backup
const sampleData2 = {
  entries: { 0: { uid: 0, content: "Modified entry.", key: ["test"] } },
};
...
assert(
  history2[0].timestamp === backup2.timestamp,
  "Newest backup is first in history",
);
...
for (let i = 0; i < 7; i++) {
  createBackup(
    TEST_BOOK,
    { entries: { i: { uid: i, content: `Entry ${i}` } } },
    3,
  );
}
const limitedHistory = getBackupHistory(TEST_BOOK);
assert(
  limitedHistory.length === 3,
  "Retention limit of 3 enforced after 7 backups",
);
...
const deepBackup = retrievedHistory.find(
  (b) => b.timestamp === backupDeep.timestamp,
);
assert(
  deepBackup.data.entries["0"].content === "Will change.",
  "Backup is a deep clone, unaffected by original mutation",
);
...
assert(
  JSON.stringify(savedData) === JSON.stringify(restoreData),
