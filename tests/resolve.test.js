...
const affected = [
    ...
    { uid: 2, comment: 'Dragon B', content: 'Dragons are big lizards that breathe fire.' },
];
...
const dupePlan = JSON.stringify({
    ...
    actions: [
        { uid: 1, action: 'rewrite', newContent: 'Dragons are big lizards that breathe fire.', reason: 'Merged details from B.' },
...
assert(dupe.actions[0].action === 'rewrite' && dupe.actions[0].newContent.length > 0, 'Rewrite action keeps newContent');
...
assert(parseResolveResponse(badAction, affected).actions[0].action === 'keep', 'Unknown action coerced to "keep"');
...
assert(parseResolveResponse(emptyRewrite, affected).actions[0].action === 'keep', 'Empty rewrite downgraded to "keep"');
...
assert(parseResolveResponse(stringUid, affected).actions[0].uid === 2, 'String uid coerced to number');
...
const strayUid = JSON.stringify({
    ...
    actions: [
        ...
        { uid: 999, action: 'delete', newContent: '', reason: 'not part of this issue' },
    ],
});
const stray = parseResolveResponse(strayUid, affected);
assert(stray.actions.length === 1 && stray.actions[0].uid === 1, 'Drops actions for uids not in the issue');

// Bare array of actions (no wrapper object) is accepted
const bareArray = JSON.stringify([
    { uid: 1, action: 'delete', newContent: '', reason: 'x' },
]);
assert(parseResolveResponse(bareArray, affected).actions.length === 1, 'Accepts a bare actions array');

// Differently-named array property is accepted
const altKey = JSON.stringify({ plan: [{ uid: 1, action: 'delete', newContent: '', reason: 'x' }] });
assert(parseResolveResponse(altKey, affected).actions.length === 1, 'Accepts a differently-named array property');

// Code-fenced response is parsed
const fenced = '```json\n' + dupePlan + '\n```';
assert(parseResolveResponse(fenced, affected).actions.length === 2, 'Parses a code-fenced resolution');

// Missing actions array throws
assertThrows(() => parseResolveResponse('{"summary": "s"}', affected), 'Throws when no actions array is present');
assertThrows(() => parseResolveResponse('not json', affected), 'Throws on non-JSON');

// All-keep / all-stray (no usable actions) throws
assertThrows(
    () => parseResolveResponse(JSON.stringify({ summary: 's', actions: [{ uid: 999, action: 'delete', newContent: '', reason: 'x' }] }), affected),
