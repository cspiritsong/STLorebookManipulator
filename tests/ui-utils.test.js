...
const sampleEntries = [
  {
    comment: "Dragon Lore",
    key: ["dragon", "wyrm"],
    content: "Ancient fire-breathing creature of the mountains.",
  },
  {
    comment: "Elven Kingdom",
    key: ["elves", "forest"],
    content: "Hidden realm deep in the enchanted forest.",
  },
  {
    comment: "Dark Mage",
    key: ["wizard", "evil"],
    content: "A powerful sorcerer who serves the dragon lord.",
  },
];
...
assert(result1.has(issueA), "Fixed issue is included in result");
assert(
  result1.has(issueB),
...
assert(!result1.has(issueC), "Cascade does not include unrelated issue C");
...
assert(result2.has(issueB), "Already-fixed set is preserved in result");
assert(result2.has(issueC), "Newly fixed issue C is added");
assert(
  result2.size === 2,
...
const result3 = computeCascadeFixedIssues(
  issueWithNull,
  [issueA, issueB, issueWithNull],
  new Set(),
);
assert(!result3.has(issueA), "Null uid is not used for cascade matching");
assert(result3.has(issueB), "Cascade matches uid 3 (ignoring null)");

// Single entry issue
const singleIssue = makeIssue([99], "Single");
const unrelatedIssue = makeIssue([100], "Unrelated");
const result4 = computeCascadeFixedIssues(
  singleIssue,
  [singleIssue, unrelatedIssue],
  new Set(),
);
assert(
  result4.size === 1,
...
// Same UID in different lorebooks must never cascade.
const bookAIssue = { ...makeIssue([5], "Book A"), bookName: "Book A" };
const bookBIssue = { ...makeIssue([5], "Book B"), bookName: "Book B" };
const crossBookResult = computeCascadeFixedIssues(
  bookAIssue,
  [bookAIssue, bookBIssue],
  new Set(),
);
assert(
  !crossBookResult.has(bookBIssue),
  "FIXED cascade does not cross lorebooks with duplicate UIDs",
);
...
assert(emptyUsage.bytes >= 0, "Empty store returns non-negative bytes");
assert(emptyUsage.percentage === 0, "Empty store returns 0 percentage");
assert(
  typeof emptyUsage.formatted === "string",
  "Empty store returns a string for formatted",
);
assert(typeof emptyUsage.isWarning === "boolean", "isWarning is a boolean");
assert(typeof emptyUsage.isCritical === "boolean", "isCritical is a boolean");

// Populate mock data with the correct prefix key and test again
mockStore["lorebook_manipulator_backups_testBook"] = JSON.stringify({
  backups: Array(50).fill({ timestamp: Date.now(), data: { entries: [] } }),
});
const smallUsage = getBackupStorageUsage();
assert(smallUsage.bytes > 0, "Usage reports positive bytes when data exists");
assert(
  smallUsage.formatted.includes("B") ||
    smallUsage.formatted.includes("KB") ||
    smallUsage.formatted.includes("MB"),
