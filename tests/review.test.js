...
assert(
  oversizedBatches.length === 1,
  "Single oversized entry produces one batch",
);
assert(oversizedBatches[0].length === 1, "Oversized entry is not dropped");

// Empty input produces no batches
assert(
  batchEntries([], 12000).length === 0,
  "Empty entry list produces no batches",
);
assert(
  batchEntries(null, 12000).length === 0,
  "Null entry list handled gracefully",
);
...
const allBatchesValid = mixedBatches.every(
  (b) =>
    b.length === 1 ||
    b.reduce((sum, e) => sum + e.content.length + e.comment.length + 100, 0) <=
      12000 + 6100,
);
assert(
  allBatchesValid,
  "Batches respect the character budget (allowing single oversized entries)",
);
  ...
  issues: [
    {
      type: "duplicate",
      severity: "high",
      description: "Entries 1 and 2 are the same.",
      entries: [
        { uid: 1, name: "A" },
        { uid: 2, name: "B" },
      ],
...
assert(
  parseReviewResponse(emptyReview).issues.length === 0,
  "Empty issues array yields no issues",
);
  ...
  issues: [
    {
      ...
      entries: [{ uid: 5, name: "X" }],
    },
  ],
});
const messyParsed = parseReviewResponse(messyReview);
assert(
  messyParsed.issues[0].type === "other",
  'Invalid issue type coerced to "other"',
);
assert(
  messyParsed.issues[0].severity === "medium",
  'Invalid severity coerced to "medium"',
);
...
assert(
  noDescParsed.issues.length === 1,
  "Issues with empty description are dropped",
);
assert(
  noDescParsed.issues[0].description === "Real issue.",
  "Keeps issues that have a description",
);
  ...
  issues: [
    {
      ...
      entries: [{ uid: "7", name: "Y" }],
    },
  ],
});
const stringUidParsed = parseReviewResponse(stringUidReview);
assert(
  stringUidParsed.issues[0].entries[0].uid === 7,
...
const fencedReview =
  ...
  JSON.stringify({
    issues: [
      { type: "other", severity: "low", description: "Fenced.", entries: [] },
    ],
  }) +
  "\n```";
assert(
  parseReviewResponse(fencedReview).issues.length === 1,
  "Parses code-fenced review response",
);
...
const bareArray = JSON.stringify([
  {
    type: "duplicate",
    severity: "high",
    description: "Bare array issue.",
    entries: [],
  },
]);
assert(
  parseReviewResponse(bareArray).issues.length === 1,
  "Accepts a bare top-level array",
);
  ...
  results: [
    {
      type: "overlap",
      severity: "low",
      description: "Alt-keyed issue.",
      entries: [],
    },
  ],
});
assert(
  parseReviewResponse(altKey).issues.length === 1,
...
assert(
  parseReviewResponse("[]").issues.length === 0,
  "Empty bare array yields no issues",
);

// An object with only non-array properties is genuinely unreadable -> throws
assertThrows(
  () => parseReviewResponse('{"foo": "bar"}'),
  "Throws when no array can be found",
);
assertThrows(
  () => parseReviewResponse("not json"),
  "Throws on non-JSON review response",
);
...
await (async () => {
  ...
  assert(
    res.skippedBatches === 0,
    "No batches skipped when all replies are readable",
  );
...
await (async () => {
  ...
  const goodWithIssue = JSON.stringify({
    issues: [
      {
        type: "other",
        severity: "low",
  ...
  const res = await reviewEntries(bigEntries, "", 2048, ctx);
  assert(
    res.skippedBatches === 1,
    "One unreadable batch is skipped (after retry), not fatal",
  );
  assert(
    res.issues.length >= 1,
    "Issues from the readable batches are still returned",
  );
})();
...
await (async () => {
  ...
  assert(
    res.skippedBatches === 0,
    "A batch that succeeds on retry is not skipped",
  );
})();
